...
describe('Survey Q1 Validation', () => {
  ...
  describe('Email and consent validation', () => {
    it('should show error when email is missing and user tries to proceed', async () => {
      ...
      expect(
        screen.getByText('Please provide your email to continue.')
      ).toBeInTheDocument();
    ...
    it('should show error when consent is not granted and user tries to proceed', async () => {
      ...
      const consentCheckbox = screen.getByRole('checkbox', {
        name: /i agree to receive marketing communications/i,
      });
      ...
      expect(
        screen.getByText(
          'Please provide consent for marketing communications to continue.'
        )
      ).toBeInTheDocument();
    ...
    it('should show error when both email and consent are missing', async () => {
      ...
      expect(
        screen.getByText(
          'Please provide your email and consent for marketing communications to continue.'
        )
      ).toBeInTheDocument();
    ...
    it('should proceed when both email and consent are provided', async () => {
      ...
      expect(
        screen.queryByText(/please provide.*to continue/i)
      ).not.toBeInTheDocument();

      // Should advance to next question
      await waitFor(() => {
        expect(
          screen.getByRole('heading', { level: 2, name: 'Next question' })
        ).toBeInTheDocument();
      });
    });
    ...
    it('should clear validation error when user fixes the email field', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByText('Please provide your email to continue.')
        ).not.toBeInTheDocument();
      });
    });
