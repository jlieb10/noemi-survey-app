...
describe('Survey', () => {
  ...
  test('first question shows no back link and Next/Skip buttons', () => {
    ...
    const mainHeading = screen.getByRole('heading', { level: 2, name: 'All About You' });
    ...
    expect(screen.getByLabelText(/email for exclusive offers/i)).toBeInTheDocument();
  });
  ...
  test('second question shows enabled back link and Continue/Skip buttons', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('heading', { name: 'What do you most want from your ritual?' })).toBeInTheDocument();
    });
